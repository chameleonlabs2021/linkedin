...
class EasyApplyBot:
    ...
    def __init__(self,
        ...
        filename1='pre_output.csv'

        self.uploads = uploads
        past_ids = self.get_appliedIDs(filename1)
    ...
    def start_apply(self, positions, locations, Remote):
        start = time.time()
        self.fill_data()

        combos = []
        while len(combos) < len(positions) * len(locations) * len(Remote):
            position = positions[random.randint(0, len(positions) - 1)]
            location = locations[random.randint(0, len(locations) - 1)]
            Remote = Remote[random.randint(0, len(Remote) - 1)]
            combo = (position, location, Remote)
            if combo not in combos:
                combos.append(combo)
                log.info(f"Applying to {position}: {location}")
                location = "&location=" + location
                self.applications_loop(position, location, Remote)
            if len(combos) > 500:
                break
    ...
    def applications_loop(self, position, location, Remote):
        ...
        self.browser, _ = self.next_jobs_page(position, location, Remote, jobs_per_page)
        log.info("Looking for jobs.. Please wait..")
    ...
    def next_jobs_page(self, position, location, Remote, jobs_per_page):
        self.browser.get(
...
if __name__ == '__main__':
    ...
    with open("config_US.yaml", 'r') as stream:
        try:
    ...
    assert len(parameters['Remote']) > 0
    ...
    Remote = [p for p in parameters['Remote'] if p != None]
    
    bot.start_apply(positions, locations, Remote)
